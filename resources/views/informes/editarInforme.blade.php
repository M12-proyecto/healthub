...
@section('title', 'Healthub - Editar informe')
@section('page', 'Informes')

@section('content')
<div class="row">
    <form method='POST' action='{{ route("editarInforme", $informe) }}' class="row g-3">
        @csrf
            ...
            <select class="form-select" id="inputPaciente" name="paciente_id">
                @if(count($pacientes) > 0)
                    ...
                    @foreach($pacientes as $paciente)
                        <option value="{{ $paciente->id }}" {{ old('paciente_id', $informe->paciente_id) == $paciente->id ? 'selected' : '' }}>{{ $paciente->nombre." ".$paciente->apellido1." ".$paciente->apellido2}}</option>
                    @endforeach
                @else
            ...
            <select class="form-select" id="inputMedico" name="medico_id">
                @if(count($medicos) > 0)
                    ...
                    @foreach($medicos as $medico)
                        <option value="{{ $medico->id }}" {{ old('medico_id', $informe->medico_id) == $medico->id ? 'selected' : '' }}>{{ $medico->nombre." ".$medico->apellido1." ".$medico->apellido2}}</option>
                    @endforeach
                @else
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control" id="inputCentro" name="centro" placeholder="Hospital, centro médico ..." value="{{ old('centro', $informe->centro) }}">
            @error('centro')
                <div class="alert alert-danger mt-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-md-3">
            ...
            <input type="text" class="form-control" id="inputEspecialidad" name="especialidad" placeholder="Especialidad médica ..." value="{{ old('especialidad', $informe->especialidad) }}">
            @error('especialidad')
                <div class="alert alert-danger mt-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-md-6">
            ...
            <textarea class="form-control" id="inputMotivoConsulta" name="motivo_consulta" rows="6">{{ old('motivo_consulta', $informe->motivo_consulta) }}</textarea>
        ...
        <div class="col-md-6">
            ...
            <textarea class="form-control" id="inputEnfermedadActual" name="enfermedad_actual" rows="6">{{ old('enfermedad_actual', $informe->enfermedad_actual) }}</textarea>
        ...
        <div class="col-md-6">
            ...
            <textarea class="form-control" id="inputDiagnostico" name="diagnostico" rows="6">{{ old('diagnostico', $informe->diagnostico) }}</textarea>
            @error('diagnostico')
                <div class="alert alert-danger mt-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-md-6">
            ...
            <textarea class="form-control" id="inputProcedimiento" name="procedimiento" rows="6">{{ old('procedimiento', $informe->procedimiento) }}</textarea>
            @error('procedimiento')
                <div class="alert alert-danger mt-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-md-12">
            ...
            <textarea class="form-control" id="inputTratamiento" name="tratamiento" rows="6">{{ old('tratamiento', $informe->tratamiento) }}</textarea>
            @error('tratamiento')
                <div class="alert alert-danger mt-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-12">
            <button type="submit" class="btn btn-primary" name="editarInformeForm">Editar informe</button>
        </div>
    </form>
</div>
@endsection